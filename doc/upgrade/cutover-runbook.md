...
| R10 | Käynnistä palvelu ja todenna. | `sudo systemctl start paperclip.service`, `scripts/upgrade-smoke.sh $API` ja `compare before after-rollback` (vaihe 5) |
| R11 | **Estä hylätyn portaan paluu.** R6 asetti `/opt/paperclip`in vanhaan SHA:han, joka on masterin esi-isä. Seuraava 05:00Z-päivitys vetäisi masterin (hylätyn portaan) takaisin ja käynnistäisi sen migraatioineen. Holdi estää tämän vain niin kauan kuin se on paikallaan. | 1) Holdi pysyy. 2) Revertaa porras masterista PR:llä: merge-commit-mergelle `git revert -m 1 <porras-merge-sha>`, squash-mergelle `git revert <squash-sha>` (ilman `-m`). 3) Mergeä revert. 4) Vie prod samaan tilaan: `sudo -u paperclip git -C /opt/paperclip fetch origin && sudo -u paperclip git -C /opt/paperclip reset --hard origin/master`, `pnpm install --frozen-lockfile` paperclipina ja `sudo systemctl restart paperclip.service`, sitten preflight ja smoke. 5) Poista holdi vasta, kun `git -C /opt/paperclip rev-parse HEAD` (paperclipina) on sama kuin `origin/master`. 6) **Ennen saman portaan uutta yritystä** revertaa revert (`git revert <revert-sha>`) tai luo porras uudelleen tuoreesta masterista: muuten `git merge <tagi>` sanoo "Already up to date" ja porras jää hiljaa pois. |

R9, suppressiot (idempotentti; `ON CONFLICT DO NOTHING`; keskeytyy ensimmäiseen virheeseen):

```bash
[ -s "$SNAP/window-export/outreach_suppressions.csv" ] || echo "ei suppressioita ikkunassa (tai vienti puuttuu: tarkista R2)"
